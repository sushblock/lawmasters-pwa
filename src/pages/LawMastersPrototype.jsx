...
const MatterDetailModal = lazy(() => import('../features/matters/MatterDetailModal'));
...
export default function LawMastersPrototype() {
  ...
  useEffect(() => {
    (async () => {
      ...
      let m = [];
      let i = [];
      try { m = await listMatters(); } catch { m = []; }
      try { i = await listInvoices(); } catch { i = []; }

      // If empty, seed with your exact SAMPLE_* (no normalization on seed)
      if (!m.length) {
        try { await seedMatters(SAMPLE_MATTERS); } catch { /* empty */ }
        try { m = await listMatters(); } catch { m = SAMPLE_MATTERS; }
      }
      if (!i.length) {
        try { await seedInvoices(SAMPLE_INVOICES); } catch { /* empty */ }
        try { i = await listInvoices(); } catch { i = SAMPLE_INVOICES; }
      }

      // Hearings: seed safely (store may not exist yet; ignore errors)
      try {
        const h = await listHearings();
        if (!h.length) { try { await seedHearings([]); } catch { /* empty */ } }
      } catch { /* ignore */ }
  ...
  const renderCurrentView = () => {
    switch (currentView) {
      case 'dashboard':
        return <Dashboard matters={matters} onMatterClick={handleMatterClick} />;
      case 'matters':
        return (
          <Matters
            ...
            // onDeleteMatter={async (id) => { await removeMatter(id); await refreshData(); }}
          />
        );
      case 'calendar':
        return (
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50">
      <Navigation currentView={currentView} onNavigate={setCurrentView} timer={timer} />
      ...
      <Suspense fallback={<Spinner />}>
        {showNewMatterForm && (
          <NewMatterForm
            ...
            onSave={async (data) => {
              // persist and refresh; UI already shows sample data until IDB reflects it
